...
func (p *Program) run() {
	...
	p.LogFile = mylog.InitLogger(cfg.CommonConfig.Logfile)
	...
	mylog.Log.Println("============================================================================")
	mylog.Log.Println("进行初始化...")
	mylog.Log.Println("初始化表结构...")
	...
	if cfg.CommonConfig.Initdata {
		mylog.Log.Println("初始化数据...")
		go managerConn.InitData(cfg.CliNodes)
	}
	mylog.Log.Println("监听DDL...")
	for _, cliNode := range cfg.CliNodes {
		mylog.Log.Infof("%s:%d 监听已打开...", cliNode.Host, cliNode.Port)
		go schema.ListeningBinglog(cliNode, managerConn)
	}
	mylog.Log.Infoln("全部监听已打开...")
}
